...
| Dashboard — графік газу (останні 6 місяців, total) | ✅ |
| Газ — виправлено charts (ВТВ з графіків, тільки total) | ✅ |
| Виправлено баг useState+async: аналітика газ і ел-ія (useEffect init) | ✅ |
| Dashboard — газ-графік тільки total (без ВТВ) | ✅ |
| Газ YoY — дельта-секція з підсумками років і % зміною | ✅ |

---

### Сесія 13: Фікси аналітики (2026-03-09)

#### 44. Газ — ВТВ прибрано з графіків (v0.9.3)
- ✅ `GasAnalytics.jsx` — усі charts показують тільки `total` (consumption + vtv)
- ✅ ВТВ bar прибрано з динаміки, MoM, YoY — масштаб ~2.7 м³ vs ~72000 м³ робив ВТВ невидимим
- ✅ MoM delta: залишено тільки "Загальне" (прибрано дублюючий рядок "Споживання")
- ВТВ залишається у таблиці та Excel export

---

### Сесія 14: Фікси газ-графіків (2026-03-09)

#### 46. Dashboard — газ тільки total (v0.9.4)
- ✅ `Dashboard.jsx` — газ-графік: `'Загальне'` = `total ?? consumption`, прибрано `'Споживання'` + `'ВТВ'` бари
- ✅ Легенда тепер показує одну серію без ВТВ

#### 47. GasAnalytics — YoY дельта-секція (v0.9.4)
- ✅ `GasAnalytics.jsx` — `yoyTotals` useMemo: сума за кожен рік
- ✅ Під YoY графіком: `compareYear1 — всього: X м³` | `compareYear2 — всього: X м³` + `▲/▼ X.X%`
- Аналогічно до MoM дельта-секції

#### 45. Баг ініціалізації useState з async props (v0.9.3)
- **Причина**: `useState(years[0])` обчислюється один раз при mount, коли `records=[]` → `years=[]` → state=`''`
- **Наслідок**: YoY/MoM не показували дані (шукали місяці типу `"-01"`)
- **Фікс**: `useState('')` + `useEffect(() => { setState(prev => prev || data[0]) }, [data])`
- ✅ `GasAnalytics.jsx` — yearFilter, compareYear1/2, cmpMonth1/2
- ✅ `ElectricityAnalytics.jsx` — yearFilter, compareYear1/2, pieMonth, cmpMonth1/2
